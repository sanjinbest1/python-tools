...
def generate_atr_operation_suggestion(atr_data):
    ...
    if atr_ratio > 1.2:
        ...
        suggestion_level = "观望"
        detailed_suggestion = (
            "当前ATR值较高，表明市场波动性较大。建议谨慎操作，避免在波动剧烈的市场中盲目追涨杀跌。"
            "建议收紧止损保护，适时调整仓位，降低潜在的波动风险。"
        )
    elif atr_ratio < 0.8:
        suggestion_text += (
            "\n\n📉 市场波动较小，行情较为稳定。\n"
        ...
        suggestion_level = "买入"
        detailed_suggestion = (
            "当前ATR值较低，表明市场波动性较小，行情较为平稳。适合短线操作或趋势跟踪策略，"
            "可以适度加仓或关注可能的突破机会。"
        )
    else:
        suggestion_text += (
        ...
        suggestion_level = "观望"
        detailed_suggestion = (
            "当前市场波动性较为稳定，没有出现显著的波动趋势。建议持续观察市场动向，"
            "同时结合其他指标（如ADX、RSI）确认合适的入场时机。"
        )
